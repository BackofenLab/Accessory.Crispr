...
#Type III systems up and downstream
...
# Also, a file containing protein sequences from the whole genomes of each of the organisms used to generate 
#the above file is needed
# I.e. this file should contain all protein sequences from all genomes used above, including the above sequences,
#in addition to Cas proteins, as well as everything else on each genome like house-keeping genes etc.
# The file is used to estimate the specificity and degree of coevolution of the found accessory genes by 
#estimating their background distribution
...
# In addition to the two files above, corresposing tsv files called flank5.lengths.tab and genomes.lengths.tab 
#are needed which contain lengths for each protein sequence in those files identified by the sequence ID in the 
#following format:
...
cat flank5.fasta | join -1 1 -2 1 <(sort) <(sort flank5.lengths.tab) | join -1 2 -2 1 <(sort -k2,2) 
<(sort flank5.lengths.tab) | sed 's/ /\t/g' | awk '{print $2 "\t" $1 "\t" $11 "\t" $13/$14 "\t" 
($8-$7)/(2*$13)+($10-$9)/(2*$14) "\t" ($7+$8-$9-$10)/($13+$14)}' | awk '{if ($3 <= 1) print}' |
awk '{if ($5 >= 0.4) print}' | awk '{if (sqrt(($4-1)^2) - (sqrt(sqrt($5))-.8) + sqrt($6^2) <= 0.1) print $1 "\t" $2}' |
mcl - -o - --abc -I 1.2 | perl -lane '$j++; $i = 0; while ($i <=$#F and $#F > 1) {$hash{$F[$i]} = $j; $i++} END {foreach $key (keys %hash)
{print "$key\t$hash{$key}"}}' | sort -k2,2n -k1,1 > flank5.clusters.tab
# the clustering result is in the above output file

# In order to proceed through the next steps it will be necessary to create individual subsets of flank5.faa 
#corresponding to each cluster in the above file and place them in a folder called "clusters/"
# I.e. for each cluster 1 to n in flank5.clusters.tab, make a file clusters/cluster1.faa to clusters/clustern.faa
#containing sequences for each protein cluster. The sequences can be extracted directly from flank5.faa.
# Once this is done you're ready to proceed to the next steps which will calculate the self similarity 
#and coevolution/specificity scores:
